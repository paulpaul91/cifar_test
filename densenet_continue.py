...
def train(net, train_data, valid_data, num_epochs, lr, wd, ctx, lr_period, lr_decay):
    ...
    for epoch in range(num_epochs):
        ...
        if((valid_data is not None) and (valid_acc>0.965)):
            break
        if((valid_data is None) and (train_acc / len(train_data))>0.9999999999):
        	break
import mxnet as mx
import resnet
ctx=mx.gpu(0)
...
lr_decay = 0.1
num_outputs=10
net = resnet.ResNet164_v2(num_outputs)
net.hybridize()   
net.load_params('resnet.params', ctx=ctx)
...
df.to_csv('submissiondensenet_continue.csv', index=False)
